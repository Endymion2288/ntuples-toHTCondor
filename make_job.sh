...
# The job directory contains a job file and a runReadTree.C which is a ROOT macro.
...
# Check if the runReadTree.C file exists:
if [ ! -f runReadTree_template.C ]; then
  echo "Error: runReadTree.C file does not exist!"
  exit 1
fi
...
while read line; do
    ...
    sed -e "s|JOB_DIR|$jobdir|g" run_root.sh > run_root.sh.new
    mv run_root.sh.new run_root.sh
    chmod +x run_root.sh

    # Create a ROOT macro from the template: runReadTree.C
    # Use sed to replace the input file name in the template with the actual input file name:
    ...
    sed -e "s|JOB_DIR|$jobdir|g" runReadTree.C > runReadTree.C.new
    mv runReadTree.C.new runReadTree.C

    # Generate the corresponding submit file: job_FILE_TIME.sub
    ...
    sed -e "s|JOB_DIR|$jobdir|g" ../job_template.sub > job_$jobdir.sub
    sed -e "s|JOB_INPUT_FILE|$line|g" job_$jobdir.sub > job_$jobdir.sub.new
    sed -e "s|JOB_EXECUTABLE|run_root.sh|g" job_$jobdir.sub.new > job_$jobdir.sub
